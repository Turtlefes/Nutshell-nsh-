void handle_builtin_export(const std::vector<std::string> &tokens)
{
    ...
    {
        std::cout << "export: export [-fn] [name[=value] ...] or export -p\n"
    ...
    for (size_t i = 1; i < tokens.size(); ++i)
    {
        ...
        {
            function_flag = true;
            std::cerr << "nsh: export: -f: shell functions not yet implemented" << std::endl;
            last_exit_code = 1;
            return;
        }
        else if (token == "--")
        ...
        else if (token[0] == '-')
        {
            ...
            std::cerr << "export: usage: export [-fn] [name[=value] ...] or export -p" << std::endl;
            last_exit_code = 2;
            return;
        }
        else
    ...
    if (remove_export)
    {
        for (const auto &name : names_to_unexport)
        {
            ...
            {
                std::cerr << "nsh: export: " << name << ": not an exported variable" << std::endl;
                last_exit_code = 1;
            }
        }
        last_exit_code = 0;
