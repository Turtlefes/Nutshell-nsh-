...
{
    if (tokens.size() == 1 || (tokens.size() > 1 && (tokens[1] == "--help" || tokens[1] == "-h")))
    {
    ...
    const std::vector<std::string> readonly_variables = {
        "BASHOPTS", "EUID", "FUNCNAME", "GROUPS", "HISTCMD", "HOSTNAME",
        "IFS", "OPTARG", "OPTIND", "OSTYPE", "PIPESTATUS", "PPID",
        "POSIXLY_CORRECT", "SECONDS", "SHELL", "SHELLOPTS", "SHLVL",
        "SUDO_GID", "SUDO_UID", "TERM", "TIMEFORMAT", "UID", "USER", "USERNAME"
    };
    ...
    if ((unset_function && unset_variable) || (name_reference && unset_function))
    {
    ...
    for (const auto &name : names_to_unset)
    {
        ...
        if (std::find(readonly_variables.begin(), readonly_variables.end(), name) != readonly_variables.end())
        {
            std::cerr << "nsh: unset: " << name << ": cannot unset: readonly variable" << std::endl;
            last_exit_code = 1;
            continue;
        }
        ...
        if (unset_function)
        {
            ...
            std::cerr << "nsh: unset: -f: shell functions not yet implemented" << std::endl;
            last_exit_code = 1;
        }
        else if (unset_variable)
        {
            ...
            {
                std::cerr << "nsh: unset: " << name << ": cannot unset: not a variable" << std::endl;
                last_exit_code = 1;
            }
        }
        else if (name_reference)
        {
            ...
            std::cerr << "nsh: unset: -n: name references not yet implemented" << std::endl;
            last_exit_code = 1;
        }
        else
        {
            ...
            if (!unset_success)
            {
                std::cerr << "nsh: unset: " << name << ": cannot unset: not found" << std::endl;
                last_exit_code = 1;
            }
        }
    }
