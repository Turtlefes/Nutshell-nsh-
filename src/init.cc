...
void initialize_session_manager() {
  ...
  if (current_session_number > to_show && should_meme > 150 &&
      isatty(STDIN_FILENO)) {
    print_session_meme();
  }

  // std::cout << "Session ID: " << new_session_id << " (Number: " <<
  // current_session_number << ")" << std::endl;
}
...
void print_session_meme() {
  std::vector<std::string> memes = {
      ...
      "Welcome, clone number " + std::to_string(current_session_number) +
          "!. Dont kill your original host.",
      ...
      "Please dont use this to kill my original bro!, i will give you some "
      "superpowers! (expecting Lie)",
      "Whatever.",
      "Session " + std::to_string(current_session_number) +
          " reporting for duty!",
      "Another one? DJ Khaled would be proud.",
      "I see you like having multiple versions of me around...",
      "Parallel universe activated: Timeline " +
          std::to_string(current_session_number),
      "Doppelganger detected! Welcome session " +
          std::to_string(current_session_number),
      "I'm not angry, just disappointed. Session " +
          std::to_string(current_session_number),
      "The more the merrier! Session count: " +
          std::to_string(current_session_number),
      "I hope you know what you're doing with all these sessions...",
      "Session proliferation detected. Initiating meme protocol.",
      "A wild session appeared! Go, Session #" +
          std::to_string(current_session_number) + "!",
      "Did you hear that? It's the sound of a new shell starting.",
      "The timeline is getting crowded. Maybe take a break?",
      "Why settle for one when you can have " +
          std::to_string(current_session_number) + "?",
      "Hello there. General Kenobi. (Session " +
          std::to_string(current_session_number) + " speaking)",
      "Is this the real life? Is this just fantasy? Caught in a new session...",
      "I have a bad feeling about this. (Just kidding, welcome " +
          std::to_string(current_session_number) + ")",
      "My shell senses are tingling! A new clone is among us.",
      "You're not you when you're hungry. You're just another session.",
      "Warning: Too many shells may cause existential dread. Proceed with "
      "caution.",
      "We are " + std::to_string(current_session_number) +
      ...
      "Did you come here to ask me for money? Because I'm broke. (Session " +
          std::to_string(current_session_number) + ")",
      "Have you tried turning it off and on again? (Not this session, the "
      "other one).",
      "It's over " + std::to_string(current_session_number) +
          "! I have the high ground!",
      "Be careful not to cross the streams. Session " +
          std::to_string(current_session_number) + " is watching.",
      "I'm pretty sure this is how Skynet started. Hi, session " +
          std::to_string(current_session_number) + ".",
...
void print_session_status() {
  ...
  std::cout << "Current Session: " << getpid()
            << " (Number: " << current_session_number << ")" << std::endl;
...
void initialize_environment() {
  ...
  LOGICAL_PWD = (pwd_env && fs::exists(pwd_env))
                    ? fs::path(pwd_env).lexically_normal()
                    : fs::current_path();
  ...
  try {
    ...
    std::cerr << "nsh: FATAL: Failed to create config directory: " << e.what()
              << std::endl;
    exit_shell(EXIT_FAILURE);
  }
...
void load_configuration() { load_aliases(); }
