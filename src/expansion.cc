...
// ... (fungsi cleanup_pattern, match_wildcard, expand_wildcard, expand_tilde
// tetap sama) ...
...
std::vector<std::string> expand_wildcard(const std::string &pattern) {
  fs::path p(pattern);
  fs::path dir = p.has_parent_path() && !p.parent_path().empty()
                     ? p.parent_path()
                     : LOGICAL_PWD;
  std::string filename_pattern = p.filename().string();
  if (filename_pattern.find_first_of("*?") == std::string::npos &&
      fs::exists(p))
    return {pattern};
  ...
  try {
    ...
    for (const auto &entry : fs::directory_iterator(dir)) {
      ...
      if (filename[0] == '.' &&
          (cleaned_pattern.empty() || cleaned_pattern[0] != '.'))
        continue;
      if (match_wildcard(cleaned_pattern.c_str(), filename.c_str())) {
        fs::path match_path = p.has_parent_path() ? (p.parent_path() / filename)
                                                  : fs::path(filename);
...
std::string expand_tilde(const std::string &path) {
  ...
  std::string user = path.substr(
      1, slash_pos == std::string::npos ? std::string::npos : slash_pos - 1);
...
double
evaluate_postfix(const std::vector<std::string>
                     &postfix); // forward declaration for tokenize_arithmetic
...
// FIX: Shunting-yard algorithm now correctly identifies and handles unary
// operators
std::vector<std::string>
infix_to_postfix(const std::vector<std::string> &tokens) {
...
// ADD: Menambahkan error handling untuk operator integer-only dalam konteks
// desimal.
double evaluate_postfix(const std::vector<std::string> &postfix) {
  ...
  for (const auto &token : postfix) {
    if (get_precedence(token) > 0) {
      ...
      if (token == "u+" || token == "u-" || token == "!" || token == "~") {
        if (stack.empty())
          throw std::runtime_error(
              "Invalid expression: not enough operands for unary op");
        double a = stack.top();
        stack.pop();
        if (token == "u+")
          stack.push(a);
          ...
          stack.push(!a);        // Logical NOT
        else if (token == "~") { // Bitwise NOT
          if (!is_whole_number(a))
            throw std::runtime_error(
                "Bitwise operator '~' requires an integer operand");
      ...
      // Logical/comparison operators
        ...
        stack.push(std::abs(a - b) < epsilon);
      else if (token == "!=")
        stack.push(std::abs(a - b) >= epsilon);
      else if (token == "&&")
        stack.push(a && b);
      ...
      if (token == "?") {
        ...
        throw std::runtime_error(
            "Puzzle variable '?' encountered during direct evaluation");
      }
  ...
  if (stack.size() != 1)
    throw std::runtime_error(
        "Invalid expression: stack should contain a single value at the end");
  return stack.top();
}

// --- GANTI FUNGSI solve_puzzle ---
// UPGRADE: Menggunakan metode aljabar linier (two-point form) untuk solusi
// instan. SUPPORT: Mendukung multiple '?' sebagai variabel yang sama. SUPPORT:
// Mendukung puzzle desimal. ADD: Error handling untuk kontradiksi dan
// identitas.
double solve_puzzle(const std::vector<std::string> &tokens) {
  ...
  // Fungsi helper untuk mengevaluasi ekspresi dengan mengganti '?' dengan nilai
  // tertentu
  auto evaluate_with_value = [&](const std::vector<std::string> &expr_tokens,
                                 double value) -> double {
    ...
    if (has_unsupported_op) {
      throw std::runtime_error(
          "Puzzles with bitwise or modulo operators are not supported");
    }
    auto postfix = infix_to_postfix(substituted_tokens);
    return evaluate_postfix(postfix);
  };

  std::vector<std::string> left_tokens(tokens.begin(),
                                       tokens.begin() + equal_pos);
  std::vector<std::string> right_tokens(tokens.begin() + equal_pos + 1,
                                        tokens.end());

  // Hitung persamaan f(x) = L(x) - R(x), di mana kita mencari x agar f(x) = 0
  // Asumsikan persamaan ini linear: f(x) = ax + b
  // Kita bisa menemukan 'a' dan 'b' dengan mengevaluasi pada dua titik, misal
  // x=0 dan x=1. f(0) = a*0 + b = b f(1) = a*1 + b = a + b Maka, a = f(1) -
  // f(0) dan b = f(0)
  ...
  try {
    ...
    throw std::runtime_error(
        std::string("Failed to evaluate puzzle structure: ") + e.what());
  }
...
std::string evaluate_arithmetic(const std::string &expr) {
  ...
  try {
    ...
    std::cerr << "nsh: arithmetic error: " << expr << ": " << e.what()
              << std::endl;
...
std::string expand_argument(const std::string &token) {
  ...
  // ADD: Variabel untuk menghasilkan angka random yang konsisten dalam satu
  // ekspansi
  static unsigned int random_seed =
      static_cast<unsigned int>(time(nullptr)) + getpid();
  ...
  for (size_t i = 0; i < token.length(); ++i) {
    ...
    if (c == '$' && !in_single_quote) {
      ...
      if (token.compare(start, 5, "RANDOM") == 0 &&
          (start + 5 >= token.length() ||
           !isalnum(token[start + 5]) && token[start + 5] != '_')) {
        ...
        i = start + 5;
                 ...
                 (start + 3 >= token.length() ||
                  !isalnum(token[start + 3]) && token[start + 3] != '_')) {
        uid_t uid = getuid();
        result += std::to_string(uid);
        i = start + 3;
      } else if (token.compare(start, 4, "EUID") == 0 &&
                 (start + 4 >= token.length() ||
                  !isalnum(token[start + 4]) && token[start + 4] != '_')) {
        uid_t euid = geteuid();
        ...
        if (start + 1 < token.length() && token[start + 1] == '(') // Arithmetic
        {
          ...
          if (end < token.length() - 1 && token[end] == ')' &&
              token[end + 1] == ')') {
        ...
        } else // Subshell
        {
          ...
          if (end < token.length() && token[end] == ')') {
            result += execute_subshell_command(
                token.substr(start + 1, end - start - 1));
            i = end;
          } else {
            result += "$(";
            i = start;
...
void apply_expansions_and_wildcards(std::vector<std::string> &tokens) {
  ...
  for (size_t i = 0; i < tokens.size(); ++i) {
    ...
    if (expanded_arg.find_first_of("*?") != std::string::npos &&
      ...
      std::vector<std::string> expanded_wildcard =
          expand_wildcard(expanded_arg);
      new_tokens.insert(new_tokens.end(), expanded_wildcard.begin(),
                        expanded_wildcard.end());
    } else {
