...
void write_job_controle_file(const Job &job) {
  ...
  if (!ofs.is_open()) {
    std::cerr << "nsh: Error writing job controle file for PGID " << job.pgid
              << std::endl;
    return;
  }
...
std::string job_status_to_string_simple(JobStatus status) {
  return status == JobStatus::RUNNING ? "Running" : "Stopped";
}
...
void check_child_status() {
  ...
  while ((pid = wait4(-1, &status, WNOHANG | WUNTRACED | WCONTINUED, &usage)) >
         0) {
 ...
 *
 * This function actively queries the OS to determine the real status of each
 * job's process group. It updates the job status in memory, moves completed
 * jobs to the finished queue, and removes stale control files for non-existent
 * processes.
 */
void validate_and_cleanup_jobs() {
  ...
  for (auto it = jobs.begin(); it != jobs.end();) {
    ...
    if (result > 0) { // A status change was detected
      if (WIFEXITED(status)) {
        job.status =
            (WEXITSTATUS(status) == 0) ? JobStatus::DONE : JobStatus::EXITED;
    ...
    if (job.status == JobStatus::DONE || job.status == JobStatus::EXITED ||
      ...
      finished_jobs.push_back(*it);
      write_job_controle_file(
          job); // This will delete the control file for a finished job.
      it = jobs.erase(it);
    } else {
      // The job is still active (Running or Stopped), so update its control
      // file.
  ...
  // This finds control files for jobs that are not in memory, possibly left
  // from a crash.
  fs::path job_dir = ns_CONFIG_DIR / "jobs";
 ...
 */
int add_job_to_list(pid_t pgid, const std::string &command, JobStatus status,
                    bool update_current) {
...
void handle_here_document(const std::string &delimiter) {
  ...
  while (true) {
    ...
    if (!std::getline(std::cin, line)) {
      std::cerr
          << "\nnsh: warning: here-document delimited by end-of-file (wanted `"
          << delimiter << "')" << std::endl;
      break;
    }
    if (line == delimiter)
...
void handle_redirection(const SimpleCommand &cmd) {
  // Urutan eksekusi redirection sangat penting. Loop ini memprosesnya sesuai
  // urutan di command line.
  for (const auto &redir : cmd.redirections) {
    ...
    case RedirectionType::HERE_DOC: // Asumsikan here-doc sudah diproses dan
                                    // filenya siap
    {
      std::string file_to_open = redir.target_file;
      if (redir.type == RedirectionType::HERE_DOC) {
        // Anda harus memodifikasi handle_here_document agar mengembalikan nama
        // file temporer Untuk saat ini, kita asumsikan namanya "heredoc.tmp"
        // seperti kode Anda yang ada
    ...
    case RedirectionType::REDIR_OUT_ERR_APPEND: {
      ...
      // &> dan &>>
      if (dup2(fd_out, 1) == -1) {
        perror("nsh: dup2 failed for stdout");
        exit_shell(1);
      }
      if (dup2(fd_out, 2) == -1) {
        perror("nsh: dup2 failed for stderr");
        exit_shell(1);
      }
      close(fd_out);
      break;
    ...
    case RedirectionType::DUPLICATE_IN:  // <&fd
    {
      if (dup2(redir.target_fd, redir.source_fd) == -1) {
        ...
        if (fcntl(redir.target_fd, F_GETFL) == -1 && errno == EBADF) {
          std::cerr << "nsh: " << redir.target_fd << ": bad file descriptor"
                    << std::endl;
        } else {
...
std::string find_binary(const std::string &cmd) {
  ...
  if (is_hashed != binary_hash_loc.end()) {
    ...
    if (fs::exists(is_hashed->second.path) &&
        fs::is_regular_file(is_hashed->second.path) &&
      ...
      std::cerr << "nsh: hash invalid: " << cmd << " entry not found"
                << std::endl;
...
std::vector<char *> build_envp() {
  ...
  for (const auto &pair : environ_map) {
    if ((pair.second.is_exported || pair.second.is_default) &&
        !pair.second.value.empty()) {
      ...
      if (!dup_str) {
        ...
        std::cerr << "nsh: Memory allocation failed for environment"
                  << std::endl;
        return {nullptr};
      }
...
void launch_process(pid_t pgid, const SimpleCommand &cmd, bool foreground,
  ...
  char **argv = static_cast<char **>(
      safe_malloc((cmd.tokens.size() + 1) * sizeof(char *)));
  if (!argv) {
  ...
  try {
    ...
    */
    ...
    const std::string &error_cmd_name =
        original_cmd_name.empty() ? cmd.tokens[0] : original_cmd_name;
    bool is_hashed = binary_hash_loc.count(error_cmd_name);

    switch (errno) {
    case EACCES:
      if (is_hashed)
        std::cerr << "nsh: hash corrupted: " << error_cmd_name
                  << ": Permission denied" << std::endl;
      else
        std::cerr << "nsh: " << error_cmd_name << ": Permission denied"
                  << std::endl;
      exit_shell(126);
    case ENOENT:
      std::cerr << "nsh: " << error_cmd_name << ": No such file or directory"
                << std::endl;
      exit_shell(127);
    case ENOEXEC:
      if (is_hashed)
        std::cerr << "nsh: hash corrupted: " << error_cmd_name
                  << ": Exec format error" << std::endl;
      else
        std::cerr << "nsh: " << error_cmd_name << ": Exec format error"
                  << std::endl;
      exit_shell(126);
    case EISDIR:
      if (is_hashed)
        ...
        std::cerr << "nsh: " << error_cmd_name << ": Is a directory"
                  << std::endl;
      exit_shell(126);
    case ENOTDIR:
      std::cerr << "nsh: " << error_cmd_name << ": Not a directory"
                << std::endl;
      exit_shell(126);
    case E2BIG:
      std::cerr << "nsh: " << error_cmd_name << ": Argument list too long"
                << std::endl;
      exit_shell(126);
    case ELOOP:
      std::cerr << "nsh: " << error_cmd_name
                << ": Too many levels of symbolic links" << std::endl;
      exit_shell(126);
    default:
      std::cerr << "nsh: " << error_cmd_name << ": " << strerror(errno)
                << std::endl;
      exit_shell(126);
    }
...
bool is_builtin(const std::string &command) {
  static const std::set<std::string> builtins = {
      "exit", "cd",   "alias",  "unalias",  "history", "pwd",   "jobs", "fg",
      "bg",   "kill", "export", "bookmark", "exec",    "unset", "hash", "type"};
...
void handle_builtin_type(const std::vector<std::string> &tokens) {
  ...
  for (const auto &name : names_to_check) {
      ...
      while (std::getline(bookmark_file, line)) {
        ...
        if (space_pos != std::string::npos) {
          ...
          if (bookmark_name == name) {
            std::cout << name << " is a bookmark to `" << bookmark_path << "`"
                      << std::endl;
            found = true;
            bookmark_file.close();
            if (!find_all)
    ...
    // 5. Cek sebagai fungsi shell (tidak diimplementasikan di sini, tapi bisa
    // ditambahkan) Saat ini tidak ada kode untuk fungsi shell di file yang
    // diberikan.
    ...
    if (hash_it != binary_hash_loc.end()) {
      std::cout << name << " is hashed (" << hash_it->second.path << ")"
                << std::endl;
      found = true;
      if (!find_all)
        continue;
...
int execute_job(const ParsedCommand &cmd_group, bool use_env) {
  ...
  if (cmd_group.pipeline.size() == 1 && !cmd_group.pipeline[0].tokens.empty() &&
  ...
  for (auto &simple_cmd : pipeline_with_paths) {
    ...
    if (original_name.find('/') != std::string::npos) {
      ...
      if (stat(original_name.c_str(), &path_stat) != 0) {
        std::cerr << "nsh: " << original_name << ": " << strerror(errno)
                  << std::endl;
        return (errno == ENOENT) ? 127 : 126;
      }

      if (S_ISDIR(path_stat.st_mode)) {
        std::cerr << "nsh: " << original_name << ": Is a directory"
                  << std::endl;
        return 126;
      }
      ...
      if (access(original_name.c_str(), X_OK) != 0) {
        if (errno == ELOOP)
          std::cerr << "nsh: " << original_name
                    << ": Too many levels of symbolic links" << std::endl;
        else
          std::cerr << "nsh: " << original_name << ": Permission denied"
                    << std::endl;
        return 126;
      }
      binary_path = fs::absolute(original_name).string();
    } else {
      // Jika bukan path, cari menggunakan find_binary (yang juga menangani
      // hash)
      binary_path = find_binary(original_name);

      if (binary_path.empty()) {
        std::cerr << "nsh: " << original_name << ": command not found"
                  << std::endl;
        return 127;
      }
    }
...
int execute_command_list(const std::vector<ParsedCommand> &commands,
  ...
  for (size_t i = 0; i < commands.size(); ++i) {
    ...
    ParsedCommand expanded_cmd = cmd_group;
    ...
    for (auto &simple_cmd : expanded_cmd.pipeline)
    {
        apply_expansions_and_wildcards(simple_cmd.tokens);
    }
    ...
    if (cmd_group.pipeline.size() == 1 &&
        ...
        !cmd_group.pipeline[0].env_vars.empty()) {
    ...
    current_exit_code = execute_job(expanded_cmd, use_env);
    last_exit_code = current_exit_code;
