...
void run_nutshell_easter_egg() {
  ...
  const std::vector<std::string> nut_frames = {
      ...
      "/\\___________/\\", "\\  _________  /", " (   o   o   ) ",
      "  |    v    |  ", "  \\_       _/  "};

  // Konfigurasi Kacang Hidup
  const std::string alive_nut_top = "\t\t/\\___________/\\ \n"
                                    "\t\t\\  _________  /\n";
  const std::string alive_nut_face = "\t\t (   o   o   ) \n"
                                     "\t\t  |    v    |  \n"
                                     "\t\t  \\_       _/  ";

  // Teks random dari kacang hidup
  const std::vector<std::string> random_texts = {
      "Did you just crack my shell?",     "Welcome to the nutshell!",
      "I'm more than just a nut.",        "The kernel of truth is here.",
      "Shell-ebrate good times!",         "Why so serious?",
      "C++ is my favorite flavor.",       "Don't worry, be happy nut.",
  ...
  for (int i = 0; i < 5; ++i) { // Kurangi iterasi untuk menghindari penumpukan
    ...
    std::cout << alive_nut_top << alive_nut_face << "\n";
...
void show_program_help() {
  ...
  std::cout << "Usage: " << program_name << " [OPTIONS] [FILE]\n\n"
            << shell_desc << "\n"
            << "\nOptions:\n"
            << std::left << std::setw(30) << "  -c, --command COMMAND"
            << "Execute COMMAND and exit\n"
            << std::setw(30) << "  -f, --file FILE"
            << "Execute commands from file targets\n"
            << std::setw(30) << "  -h, --help" << "Show this help message\n"
            << std::setw(30) << "  -v, --version"
...
void execute_script_file(const std::string &filename) {
  ...
  bool has_nsh_shebang =
      (first_line.find("#!/bin/nsh") != std::string::npos ||
  ...
  if (fs::exists(rcpath) && fs::is_regular_file(rcpath)) {
    ...
    if (file.is_open()) {
      ...
      while (std::getline(file, line)) {
        ...
        try {
          auto commands = parser.parse(line);
          if (!commands.empty()) // id command not empty, jika command tidak
                                 // kosong maka
          {
            last_exit_code = execute_command_list(
                commands); // maka exit code menyesuaikan return fungsi
                           // execute_command_list, soalnya fungsi itu bertipe
                           // int dan bisa return
          }
        } catch (const std::exception &e) {
          std::cerr << "error while processing rcfile: " << e.what()
                    << std::endl;
        }
      }
    } else {
...
void report_finished_jobs() {
  ...
  for (const auto &finished_job_pair : finished_jobs) {
    ...
    std::cout << "[" << id << "]+" << "\t" << std::left << std::setw(10)
  ...
  while (true) {
    ...
    if (!dont_execute_first) {
      try {
        ...
        // Perubahan di sini: Hanya laporkan finished_jobs jika ada perintah
        // yang dijalankan
        if (!commands.empty()) {
          last_exit_code = execute_command_list(commands);
