void load_history_append() {
  ...
  if (history_file.is_open()) {
    ...
    while (std::getline(history_file, line)) {
      if (std::find(command_history.begin(), command_history.end(), line) ==
          command_history.end()) {
...
void save_history_replace() { save_history(); }
...
void handle_builtin_history(const std::vector<std::string> &tokens) {
  if (tokens.size() > 1 && (tokens[1] == "--help" || tokens[1] == "-h")) {
    std::cout
        ...
        << "    Options:\n"
        << "      -c        clear the history list by deleting all of the "
           "entries\n"
        << "      -d offset delete the history entry at position OFFSET. "
           "Negative\n"
        << "                offsets count back from the end of the history "
           "list\n"
        << "      -a        append history lines from this session to the "
           "history file\n"
        << "      -n        read all history lines not already read from the "
           "history file\n"
        << "                and append them to the history list\n"
        << "      -r        read the history file and append the contents to "
           "the history\n"
        << "                list\n"
        << "      -w        write the current history to the history file\n"
        << "      -p        perform history expansion on each ARG and display "
           "the result\n"
        << "                without storing it in the history list\n"
        << "      -s        append the ARGs to the history list as a single "
           "entry\n\n"
        << "    If FILENAME is given, it is used as the history file. "
           "Otherwise\n"
        << "    if $HISTFILE has a value, that is used, else "
           "~/.nsh/history.\n\n"
        << "    Exit Status:\n"
  ...
  for (size_t i = 1; i < tokens.size(); ++i) {
    ...
    if (token == "-c") {
      ...
      std::cerr << "history: use 'history --help' for more information."
                << std::endl;
      last_exit_code = 1;
      return;
    } else {
  ...
  if (delete_entry) {
    ...
    if (actual_index >= 0 && actual_index < history_length) {
      ...
      if (entry) {
        free(entry->line);
        free(entry);
        std::cout << "Deleted history entry " << (actual_index + 1)
                  << std::endl;
      }
    } else {
      std::cerr << "history: offset out of range: " << delete_offset
                << std::endl;
      last_exit_code = 1;
    }
    free(hist_state);
