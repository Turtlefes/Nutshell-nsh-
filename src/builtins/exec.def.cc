...
// Asumsi fungsi-fungsi berikut dideklarasikan di tempat lain (misalnya,
// execution.h) dan dapat diakses oleh file ini. extern int last_exit_code;
...
void handle_builtin_exec(const std::vector<std::string> &tokens) {
  ...
  if (tokens[1] == "--help" || tokens[1] == "-h") {
    std::cout
        << "exec: exec [-clv] [-a name] [command [arguments ...]] [redirection "
           "...]\n"
        ...
        << "    If COMMAND cannot be executed, the shell exits with non-zero "
           "status.\n\n"
        << "    Exit Status:\n"
        << "    Returns success unless COMMAND is not found or an error "
           "occurs.\n";
    last_exit_code = 0;
    return;
  }
  ...
  while (i < tokens.size()) {
    ...
    if (token == "<" || token == ">" || token == ">>" || token == "2>" ||
      ...
      if (i + 1 < tokens.size()) {
        ...
        std::cerr << "nsh: exec: syntax error: missing file for redirection"
                  << std::endl;
        last_exit_code = 2;
        return;
      }
  ...
  auto apply_redirections = [&]() -> bool {
    for (size_t k = 0; k < redirections.size(); k += 2) {
      ...
      if (op == "<") {
        ...
        flags = (op == "&>") ? (O_WRONLY | O_CREAT | O_TRUNC)
                             : (O_WRONLY | O_CREAT | O_APPEND);
      }

      int fd = open(file.c_str(), flags, 0666);
      if (fd == -1) {
        std::cerr << "nsh: exec: " << file << ": " << strerror(errno)
                  << std::endl;
        last_exit_code = 1;
        return false;
      }
  ...
  if (binary_path.empty()) {
    std::cerr << "nsh: exec: " << command_args[0] << ": command not found"
              << std::endl;
    last_exit_code = 127;
  ...
  if (execve(binary_path.c_str(), argv, envp) == -1) {
    std::cerr << "nsh: exec: " << command_args[0] << ": " << strerror(errno)
              << std::endl;
