...
void handle_builtin_hash(const std::vector<std::string> &tokens) {
  if (tokens.size() > 1 && (tokens[1] == "--help" || tokens[1] == "-h")) {
    std::cout
        ...
        << "    Determine and remember the full pathname of each command NAME. "
           " If\n"
        << "    no arguments are given, information about remembered commands "
           "is displayed.\n\n"
        << "    Options:\n"
        ...
        << "    Arguments:\n"
        << "      NAME\tEach NAME is searched for in $PATH and added to the "
           "list\n"
        << "\t\tof remembered commands.\n\n"
        << "    Exit Status:\n"
        << "    Returns success unless NAME is not found or an invalid option "
           "is given.\n";
    last_exit_code = 0;
    return;
  }
    ...
    if (parsing_options && token.size() > 1 && token[0] == '-') {
      ...
      for (size_t j = 1; j < token.size(); ++j) {
        ...
        default:
          ...
          std::cerr << "hash: Try 'hash --help' for more information."
                    << std::endl;
          last_exit_code = 1;
          return;
        }
  ...
  if (forget_all) {
    if (verbose) {
      std::cout << "hash: hash table emptied (" << binary_hash_loc.size()
                << " entries removed)" << std::endl;
    } else {
  ...
  if (forget_location) {
    ...
    for (const auto &name : names) {
      ...
      if (it != binary_hash_loc.end()) {
        if (verbose) {
          std::cout << "hash: " << name
                    << ": removed from hash table (was: " << it->second.path
                    << ")" << std::endl;
        } else {
          std::cout << "hash: " << name << ": removed from hash table"
                    << std::endl;
        }
  ...
  if (!custom_path.empty()) {
    ...
    if (!fs::exists(path)) {
      std::cerr << "hash: " << custom_path << ": no such file or directory"
                << std::endl;
      last_exit_code = 1;
      return;
    }
    ...
    if (!fs::is_regular_file(path)) {
      std::cerr << "hash: " << custom_path << ": not a regular file"
                << std::endl;
      last_exit_code = 1;
      return;
    }
    ...
    if (access(path.c_str(), X_OK) != 0) {
      std::cerr << "hash: " << custom_path << ": permission denied"
                << std::endl;
      last_exit_code = 1;
      return;
    }
    ...
    if (verbose) {
      std::cout << "hash: added " << names[0] << " = " << abs_path
                << " (custom path)" << std::endl;
    } else {
  ...
  if (names.empty()) {
    ...
    if (list_format) {
      ...
      for (const auto &[cmd, info] : sorted_entries) {
        std::cout << std::setw(4) << info.hits << "\t" << info.path
                  << std::endl;
      }
    }
    ...
    if (show_summary) {
      ...
      std::cout << binary_hash_loc.size() << " command(s), " << total_hits
                << " total hit(s)" << std::endl;
    }
  ...
  for (const auto &name : names) {
    ...
    if (!binary_path.empty()) {
      ...
      if (terse_format) {
        ...
        std::cout << "hash: added " << name << " = " << binary_path
                  << std::endl;
      } else {
        std::cout << name << " = " << binary_path << std::endl;
      }
  ...
  if (verbose && !names.empty()) {
    if (added_count > 0) {
      std::cout << "hash: added " << added_count << " command(s) to hash table"
                << std::endl;
    }
    if (found_count > 0) {
      std::cout << "hash: found " << found_count
                << " command(s) already in hash table" << std::endl;
    }
  }
