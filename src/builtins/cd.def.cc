void handle_builtin_cd(const std::vector<std::string> &t) {
  ...
  if (t.size() > 1 && (t[1] == "--help" || t[1] == "-help")) {
    std::cout
        ...
        << "      -e        if the -P option is supplied, and the current "
           "working directory\n"
  ...
  for (size_t i = 1; i < t.size(); ++i) {
    ...
    if (token.rfind("-", 0) == 0 && token.length() > 1) {
      if (token == "-P") {
        ...
        if (!bookmark_found) {
          std::cerr << "nsh: cd: " << token
                    << ": invalid option or bookmark not found" << std::endl;
  ...
  if (!from_bookmark && (path_arg_str == "." || path_arg_str == ".." ||
                         path_arg_str.find("../") == 0)) {
  ...
  try {
    ...
    if (use_physical) {
      ...
      physical_path_to_change = fs::canonical(
          target_path.is_absolute() ? target_path
    ...
    if (!fs::exists(physical_path_to_change)) {
      std::cerr << "nsh: cd: " << path_arg_str << ": No such file or directory"
                << std::endl;
      last_exit_code = 1;
      return;
    }
    ...
    if (!fs::is_directory(physical_path_to_change)) {
      std::cerr << "nsh: cd: " << path_arg_str << ": Not a directory"
                << std::endl;
      last_exit_code = 1;
      return;
    }
    ...
    std::cerr << "nsh: cd: " << path_arg_str << ": " << e.code().message()
              << std::endl;
    last_exit_code = 1;
