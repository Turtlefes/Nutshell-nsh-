...
void update_job_status_from_system(DisplayJobInfo &job) {
  ...
  if (result > 0) {
    ...
    // Untuk job yang di-stop, biasanya waitpid dengan WUNTRACED akan
    // mengembalikan status stopped
 ...
 */
DisplayJobInfo read_job_controle_file(const fs::path &file_path,
                                      int temp_job_id,
                                      pid_t current_shell_pid) {
  DisplayJobInfo info = {};
  ...
  while (std::getline(ifs, line)) {
    ...
    try {
      if (key == "PGID")
        ...
        info.status = static_cast<JobStatus>(std::stoi(value));
      else if (key == "TERM_STATUS")
        info.term_status = std::stoi(value);
      else if (key == "RUTIME_SEC")
        info.usage.ru_utime.tv_sec = std::stoi(value);
      else if (key == "RUTIME_USEC")
        info.usage.ru_utime.tv_usec = std::stoi(value);
      else if (key == "RSTIME_SEC")
        info.usage.ru_stime.tv_sec = std::stoi(value);
      else if (key == "RSTIME_USEC")
        info.usage.ru_stime.tv_usec = std::stoi(value);
      else if (key == "START_TIME_SEC")
        info.start_tv.tv_sec = std::stoi(value);
      else if (key == "START_TIME_USEC")
        info.start_tv.tv_usec = std::stoi(value);
    } catch (const std::exception &e) {
      std::cerr << "nsh: jobs: error parsing control file: " << e.what()
                << std::endl;
    }
  }
...
void determine_current_and_previous_jobs(std::vector<DisplayJobInfo> &jobs) {
  ...
  for (const auto &job : jobs) {
    if (job.is_current_session && (job.status == JobStatus::RUNNING ||
                                   job.status == JobStatus::STOPPED)) {
...
/**
 * @brief Mendapatkan semua job yang aktif di sistem (cross-session) tanpa
 * duplikasi
 */
  ...
  if (fs::exists(job_dir)) {
    for (const auto &entry : fs::directory_iterator(job_dir)) {
      if (entry.is_regular_file() && entry.path().extension() == ".controle") {
        DisplayJobInfo info = read_job_controle_file(
            entry.path(), next_display_id++, current_shell_pid);
        ...
        if (info.status != JobStatus::DONE &&
            info.status != JobStatus::EXITED &&
            info.status != JobStatus::SIGNALED) {
          all_jobs.push_back(info);
        }
      }
    }
 ...
 */
std::string format_cpu_percentage(const struct rusage &usage,
...
void show_jobs_help() {
  std::cout << "Usage: jobs [options]\n"
            ...
            << "Options:\n"
            << "  -l          Display process IDs and detailed information in "
               "ps-like format.\n"
...
void handle_builtin_jobs(const std::vector<std::string> &tokens) {
  ...
  for (size_t i = 1; i < tokens.size(); ++i) {
    if (tokens[i] == "-l") {
      ...
      std::cerr << "nsh: jobs: invalid option -- '" << tokens[i] << "'"
                << std::endl;
      show_jobs_help();
      return;
    }
  ...
  if (list_details) {
    std::cout << std::left << std::setw(8) << "JOBID" << std::setw(8) << "STAT"
              << std::setw(10) << "PGID" << std::setw(12) << "SESSION"
              << std::setw(12) << "CPU Time" << std::setw(8) << "CPU%"
              << "COMMAND" << std::endl;
    ...
    for (const auto &job : filtered_jobs) {
      ...
      if (job.status == JobStatus::RUNNING) {
        std::cout << std::setw(8)
                  << format_cpu_percentage(job.usage, job.start_tv);
      } else {
...
/**
 * @brief Mendapatkan nomor sinyal dari nama sinyal (case-insensitive,
 * menghilangkan awalan SIG).
...
int get_signal_by_name(const std::string &sigspec) {
  ...
  std::transform(upper_sigspec.begin(), upper_sigspec.end(),
                 upper_sigspec.begin(), ::toupper);
...
void show_kill_help() {
  std::cout
      << "Usage: kill [options] <pid>|<jobspec>...\n"
...
void handle_builtin_kill(const std::vector<std::string> &tokens) {
  ...
  if (tokens.size() < 2) {
    std::cerr << "nsh: kill: usage: kill [-s signal | -n number | -signal] pid "
                 "| jobspec ...\n";
    ...
    if (token == "-s") {
      ...
      if (i + 1 < tokens.size()) {
        ...
        try {
          ...
          std::cerr << "nsh: kill: invalid signal number: " << signum_str
                    << std::endl;
          last_exit_code = 1;
          return;
        }
      ...
      if (std::all_of(spec.begin(), spec.end(), ::isdigit)) {
        try {
          ...
          std::cerr << "nsh: kill: invalid signal number: " << spec
                    << std::endl;
          last_exit_code = 1;
          return;
        }
        ...
        if (sig != 0) {
          ...
          // Kita akan tetap pada interpretasi sinyal jika tidak ada target yang
          // ditentukan sebagai PID/PGID negatif secara eksplisit. Untuk
          // kesederhanaan, jika bukan sinyal yang dikenal, ini adalah error
          std::cerr << "nsh: kill: unknown signal or option: " << token
                    << std::endl;
          last_exit_code = 1;
          return;
        }
  ...
  for (const std::string &target : targets) {
    ...
    if (target[0] == '%') {
      ...
      if (jobspec_to_pgid(target, pgid_to_kill) == 0) {
        ...
        // Jobspec tidak valid/tidak ditemukan/ambigu (pesan error sudah di
        // dalam jobspec_to_pgid)
        error_count++;
        continue;
      }
    } else {
      ...
      try {
        ...
        if (target_id > 0) {
          ...
          // PID 0 tidak valid
          std::cerr << "nsh: kill: invalid target: " << target
                    << " (PID cannot be 0)" << std::endl;
          error_count++;
          continue;
        }
      } catch (const std::exception &) {
        std::cerr << "nsh: kill: invalid target argument: " << target
                  << std::endl;
        error_count++;
        continue;
      }
    ...
    if (kill_result == 0) {
      std::string target_desc =
          (target[0] == '%')
              ? ("Job " + target + " (PGID " +
                 std::to_string(std::abs(target_id)) + ")")
              : (target_id > 0 ? "Process " + target
                               : "Process Group " + std::to_string(-target_id));
      std::cout << target_desc << " signaled with " << strsignal(signal_num)
                << std::endl;
    } else {
      // kill_result == -1
      std::cerr << "nsh: kill: failed to send signal " << strsignal(signal_num)
                << " to " << target << ": " << strerror(errno) << std::endl;
      error_count++;
    }
  }
...
void show_fg_help() {
  std::cout << "Usage: fg [jobspec]\n"
            ...
            << "The job will resume execution in the foreground with terminal "
               "control.\n"
...
void handle_builtin_bg(const std::vector<std::string> &tokens) {
  ...
  if (!job_info->is_current_session) {
    std::cerr << "nsh: bg: cannot enforce session background control ("
              << job_info->session_display_name << ") from this session"
              << std::endl;
    last_exit_code = 1;
    return;
  }
  ...
  if (job_ptr) {
    if (job_ptr->status == JobStatus::RUNNING) {
      std::cerr << "nsh: bg: job " << job_info->job_id
                << " already in background" << std::endl;
    ...
    if (kill(-pgid_out, SIGCONT) < 0) {
      ...
      std::cout << "[" << job_info->job_id << "]+ " << job_ptr->command << " &"
                << std::endl;
      last_exit_code = 0;
    }
  } else {
    std::cerr << "nsh: bg: job not found in current session: " << jobspec
              << std::endl;
    last_exit_code = 1;
  }
}
...
void handle_builtin_fg(const std::vector<std::string> &tokens) {
  ...
  do {
    ...
    if (WIFSTOPPED(status)) {
      ...
      std::cout << "\n[" << job_id_to_erase << "]+ Stopped\t"
                << job_ptr->command << std::endl;
      break;
    } else if (WIFCONTINUED(status)) {
      job_ptr->status = JobStatus::RUNNING;
