...
void handle_builtin_fg(const std::vector<std::string>& tokens) {
    ...
    // Validasi bahwa kita berada di interactive terminal
    if (!isatty(STDIN_FILENO)) {
        std::cerr << "nsh: fg: no job control in non-interactive shells" << std::endl;
        last_exit_code = 1;
        return;
    }
    ...
    if (!job_info) {
        std::cerr << "nsh: fg: job not found: " << jobspec << std::endl;
        last_exit_code = 1;
        return;
    }

    if (!job_info->is_current_session) {
        std::cerr << "nsh: fg: job " << jobspec << " is from another session (" 
                  << job_info->session_display_name << ")" << std::endl;
        last_exit_code = 1;
        return;
    }
    ...
    if (!job_ptr) {
        std::cerr << "nsh: fg: job not found in current session: " << jobspec << std::endl;
        last_exit_code = 1;
        return;
    }

    if (job_ptr->status == JobStatus::RUNNING) {
        std::cout << "nsh: fg: job " << job_id << " already in foreground" << std::endl;
        last_exit_code = 0;
        return;
    }
    ...
    // === [V2: Perbaikan urutan terminal handling dan signals] ===

    // Simpan state terminal SEBELUM modifikasi
    struct termios saved_tmodes;
    tcgetattr(STDIN_FILENO, &saved_tmodes);

    // Block signals selama transisi kritis
    ...
    // Simpan juga mask SIGCHLD (dari V1)
    sigset_t mask_sigchld;
    sigemptyset(&mask_sigchld);
    sigaddset(&mask_sigchld, SIGCHLD);
    sigprocmask(SIG_BLOCK, &mask_sigchld, nullptr);

    // Simpan terminal mode global shell
    struct termios global_shell_tmodes = shell_tmodes;
    ...
    // Set foreground_pgid SEBELUM beri terminal
    foreground_pgid = job_ptr->pgid;
    current_job_id = job_id;

    // Berikan terminal control dulu, baru restore mode
    if (tcsetpgrp(STDIN_FILENO, job_ptr->pgid) < 0) {
        if (errno == ESRCH) {
            std::cerr << "nsh: fg: job has terminated" << std::endl;
        } else if (errno == ENOTTY) {
            std::cerr << "nsh: fg: no job control in this terminal" << std::endl;
        } else {
    ...
    // Restore terminal mode SETELAH tcsetpgrp
    ...
    // === [V1: Wait loop dan handling status job] ===
    ...
    do {
        ...
        if (w == -1) {
            if (errno == EINTR) {
                ...
                if (wait_timeout < MAX_WAIT_RETRIES) {
                    wait_timeout++;
                    usleep(100000);
                    continue;
                }
        ...
        if (WIFSTOPPED(status)) {
            ...
            job_stopped = true;
            std::cout << "\n[" << job_id << "]+  Stopped\t\t" << job_ptr->command << std::endl;
            break;
        } else if (WIFCONTINUED(status)) {
            job_ptr->status = JobStatus::RUNNING;
    ...
    } while (!job_completed && !job_stopped);

    // === [V2: Perbaikan terminal restore] ===
    foreground_pgid = 0;

    int attempts = 0;
    while (tcsetpgrp(STDIN_FILENO, shell_pgid) < 0 && attempts < 3) {
        if (errno == EINTR) {
            attempts++;
            usleep(100000);
            continue;
        ...
        if (isatty(STDIN_FILENO)) {
            perror("nsh: fg: warning: failed to restore terminal control");
        }
        break;
    }

    // Restore terminal attributes
    tcsetattr(STDIN_FILENO, TCSADRAIN, &saved_tmodes);

    // === [V1: Update job status & cleanup] ===
    if (WIFEXITED(status)) {
        int exit_code = WEXITSTATUS(status);
        job_ptr->status = (exit_code == 0) ? JobStatus::DONE : JobStatus::EXITED;
        ...
        std::cout << "\n[" << job_id << "]+  Terminated\t" 
                  << strsignal(WTERMSIG(status)) << "\t\t" << job_ptr->command << std::endl;
