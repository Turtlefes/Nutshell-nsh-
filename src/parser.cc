...
std::vector<Token> Parser::tokenize(const std::string &input) const {
  ...
  for (size_t i = 0; i < input.length(); ++i) {
    ...
    if (!in_quote && !in_arithmetic && c == '$') {
      ...
      if (i + 2 < input.length() && input[i + 1] == '(' &&
        ...
        while (i < input.length() && paren_count > 0) {
          ...
          if (paren_count == 0 && i + 1 < input.length() &&
              input[i + 1] == ')') {
            current_token += ')';
            i++;
            break;
        ...
        if (i < input.length() && input[i] == ']') {
          ...
          std::cerr
              << "nsh: syntax error: unclosed legacy arithmetic expansion\n";
          return {};
        }
        current_token.clear();
        continue;
    ...
    if (!in_quote && !in_arithmetic && c == '$' && i + 1 < input.length() &&
        input[i + 1] == '(') {
      if (!current_token.empty()) {
        tokens.push_back({is_assignment && is_env_assignment(current_token)
                              ? TokenType::ASSIGNMENT_WORD
    ...
    if (!in_quote && c == '$' && i + 1 < input.length() &&
        input[i + 1] == '{') {
      if (!current_token.empty()) {
        tokens.push_back({is_assignment && is_env_assignment(current_token)
                              ? TokenType::ASSIGNMENT_WORD
    ...
    if (c == '\'' || c == '"') {
      ...
      current_token += c;
      if (is_assignment && !in_assignment_word &&
          is_env_assignment(current_token)) {
      ...
      current_token += c;
      // Jika kita sedang dalam konteks assignment dan menemukan karakter
      // non-identifier, maka ini bukan assignment word yang valid
...
void Parser::expand_aliases(std::vector<Token> &tokens) {
  ...
  for (size_t i = 0; i < tokens.size(); ++i) {
    ...
    if (is_command_start && token.type == TokenType::WORD) {
      ...
      if (it != aliases.end() &&
          expansion_guard.find(token.text) == expansion_guard.end()) {
        ...
        if (!alias_tokens.empty()) {
          ...
          tokens.insert(tokens.begin() + i, alias_tokens.begin(),
                        alias_tokens.end());
...
std::vector<ParsedCommand> Parser::parse(const std::string &input) {
  ...
  // expect_redirect_file, last_redirect_type tidak lagi diperlukan
  ...
  for (size_t i = 0; i < tokens.size(); ++i) {
    ...
    // --- AWAL PERBAIKAN BUG REDIREKSI FD DENGAN ANGKA AWAL ---
    // Pola: WORD(angka) diikuti oleh operator GREAT, LESS, etc.
    if (token.type == TokenType::WORD && i + 1 < tokens.size() &&
        is_string_numeric(token.text)) {
      const Token &next_token = tokens[i + 1];
      if (next_token.type == TokenType::GREAT ||
          next_token.type == TokenType::DGREAT ||
          next_token.type == TokenType::LESS) {
        if (i + 2 < tokens.size()) {
          const Token &target_token = tokens[i + 2];
          Redirection redir;
          redir.source_fd = std::stoi(token.text);

          // Kasus Duplikasi FD (e.g., 2>&1) atau Penutupan FD (e.g., 2>&-)
          if (target_token.type == TokenType::AMPERSAND) {
            if (i + 3 < tokens.size()) {
              const Token &final_target = tokens[i + 3];
              if (final_target.text == "-") {
                // Menutup FD: 2>&-
                redir.type = RedirectionType::CLOSE_FD;
              } else if (is_string_numeric(final_target.text)) {
                // Duplikasi FD: 2>&1
                redir.target_fd = std::stoi(final_target.text);
                redir.type = (next_token.type == TokenType::LESS)
                                 ? RedirectionType::DUPLICATE_IN
                                 : RedirectionType::DUPLICATE_OUT;
              } else {
                std::cerr << "nsh: " << final_target.text
                          << ": ambiguous redirect" << std::endl;
                return {};
              }
              current_simple_cmd.redirections.push_back(redir);
              i += 3; // Lewati 4 token: '2', '>', '&', '1'/'--'
              continue;
            } else {
          ...
          // Kasus Pengalihan File Biasa dengan FD Spesifik (e.g., 2>file)
          else if (target_token.type == TokenType::WORD ||
                   target_token.type == TokenType::STRING) {
            ...
            current_simple_cmd.redirections.push_back(redir);
            i += 2; // Lewati 3 token: '2', '>', 'file'
            continue;
          }

          // Jika token berikutnya bukan '&' atau bukan nama file
          // Kasus error sintaks
          std::cerr << "nsh: syntax error: expected filename or FD target "
                       "after redirect"
                    << std::endl;
          return {};

        } else {
          std::cerr << "nsh: syntax error: expected target for redirection `"
                    << next_token.text << "'" << std::endl;
          return {};
        }
      }
    }
    // --- AKHIR PERBAIKAN BUG REDIREKSI FD DENGAN ANGKA AWAL ---
    ...
    case TokenType::ASSIGNMENT_WORD: {
      if (!command_word_found) {
        ...
        // more recommended, because we don't know the original exported or
        // default state
    ...
    case TokenType::PIPE:
      if (current_simple_cmd.tokens.empty() &&
          current_simple_cmd.env_vars.empty() &&
          current_simple_cmd.redirections
              .empty()) // Tambahkan cek untuk redirections juga
      {
        std::cerr << "nsh: syntax error near unexpected token `" << token.text
                  << "'" << std::endl;
        return {};
      }
    ...
    case TokenType::LESSLESSLESS: {
      // Periksa apakah token berikutnya adalah '&', menandakan duplikasi atau
      // penutupan FD
      if (i + 1 < tokens.size() && tokens[i + 1].type == TokenType::AMPERSAND) {
        // Pola: > & target
        if (i + 2 < tokens.size()) {
          const Token &target_token = tokens[i + 2];
          Redirection redir;
          redir.source_fd = (token.type == TokenType::LESS)
                                ? 0
                                : 1; // Default FD untuk < dan >

          if (target_token.text == "-") {
            // Menutup FD: >&-
            redir.type = RedirectionType::CLOSE_FD;
          } else if (is_string_numeric(target_token.text)) {
            // Duplikasi FD: >&1
            redir.target_fd = std::stoi(target_token.text);
            redir.type = (token.type == TokenType::LESS)
                             ? RedirectionType::DUPLICATE_IN
                             : RedirectionType::DUPLICATE_OUT;
          } else {
            std::cerr << "nsh: " << target_token.text << ": ambiguous redirect"
                      << std::endl;
            return {};
          }
          current_simple_cmd.redirections.push_back(redir);
          i += 2; // Lewati operator, '&', dan target
          break;
        } else {
          std::cerr << "nsh: syntax error near unexpected token `&'"
                    << std::endl;
      ...
      // Pastikan target adalah nama file/delimiter (WORD atau STRING)
      if (target_token.type != TokenType::WORD &&
          target_token.type != TokenType::STRING) {
        std::cerr << "nsh: syntax error: expected filename/delimiter after "
      ...
      Redirection redir;
      redir.source_fd =
          (token.type == TokenType::LESS || token.type == TokenType::LESSLESS ||
           token.type == TokenType::LESSLESSLESS)
              ? 0
              : 1;
    ...
    case TokenType::AMPERSAND:
      // Handle &> dan &>>
      if (i + 1 < tokens.size() && (tokens[i + 1].type == TokenType::GREAT ||
                                    tokens[i + 1].type == TokenType::DGREAT)) {
        if (i + 2 < tokens.size()) {
          const Token &target_token = tokens[i + 2];
          if (target_token.type != TokenType::WORD &&
              target_token.type != TokenType::STRING) {
            std::cerr << "nsh: syntax error: expected filename after redirect"
                      << std::endl;
            return {};
          }
          Redirection redir;
          redir.target_file = target_token.text;
          redir.type = (tokens[i + 1].type == TokenType::GREAT)
                           ? RedirectionType::REDIR_OUT_ERR
                           : RedirectionType::REDIR_OUT_ERR_APPEND;
          current_simple_cmd.redirections.push_back(redir);
          i += 2; // Lewati '&', '>', dan 'file'
        } else {
          std::cerr << "nsh: syntax error: expected filename after redirect"
      ...
      // Handle job background
        ...
        // Hanya set background jika ada command/redirection yang valid
        if (!current_simple_cmd.tokens.empty() ||
            !current_simple_cmd.env_vars.empty() ||
            !current_simple_cmd.redirections.empty() ||
            !command_list.back().pipeline.empty()) {
          command_list.back().background = true;
        } else {
          // '&' tanpa command di depan adalah error (atau diperlakukan sebagai
          // kata biasa)
          current_simple_cmd.tokens.push_back(token.text);
        }
      } else {
  ...
  command_list.erase(std::remove_if(command_list.begin(), command_list.end(),
                                    [](const ParsedCommand &cmd) {
                                      return cmd.pipeline.empty() &&
                                             !cmd.background;
                                    }), // Tambahkan pengecekan background
                     command_list.end());
...
std::string Parser::get_history_by_number(int number) {
  ...
  if (number < 0 || number >= hist_length) {
    std::cerr << "nsh: history: history position " << number << " not found"
              << std::endl;
    return "";
  }
...
std::string Parser::get_history_by_pattern(const std::string &pattern) {
  ...
  std::cerr << "nsh: history: no matching history entry for `" << pattern << "'"
            << std::endl;
  return "";
}
  ...
  while (pos < input.length()) {
    ...
    if (next_char == '!') {
      ...
      expansion_found = true;
               ...
               (next_char == '-' && bang_pos + 2 < input.length() &&
                isdigit(input[bang_pos + 2]))) {
      // !n or !-n - history number n
      size_t end_pos = bang_pos + 2;
      while (end_pos < input.length() &&
             (isdigit(input[end_pos]) || input[end_pos] == '-')) {
        end_pos++;
      }
      ...
      size_t end_pos = bang_pos + 2;
      while (end_pos < input.length() &&
             (isalnum(input[end_pos]) || input[end_pos] == '_' ||
              input[end_pos] == '-')) {
        end_pos++;
      }
    ...
    if (expansion_found && !replacement.empty()) {
      ...
      for (char c : replacement) {
        if (c == '\'' &&
            (result.size() == 1 || result[result.size() - 2] != '\\')) {
          in_single_quote = !in_single_quote;
        } else if (c == '"' &&
                   (result.size() == 1 || result[result.size() - 2] != '\\')) {
          in_double_quote = !in_double_quote;
...
bool Parser::needs_EOF_IN(const std::string &line) const {
  ...
  // Check for backslash EOF_IN - only if backslash is the last non-whitespace
  // character and it's not part of an escape sequence
