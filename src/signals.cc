...
void sigint_handler(int signum) {
    ...
    // **PERBAIKAN: Gunakan pendekatan yang lebih reliable**
    pid_t current_foreground = isatty(STDIN_FILENO) ? tcgetpgrp(STDIN_FILENO) : 0;
    
    if (current_foreground == shell_pgid || current_foreground == 0) {
        // Shell di foreground
        clear_readline_input();
    } else {
        // Process lain di foreground - kirim SIGINT
        if (kill(-current_foreground, SIGINT) < 0 && errno != ESRCH) {
            perror("nsh: kill (SIGINT)");
        }
    }
}
