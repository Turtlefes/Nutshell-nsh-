...
const std::vector<SignalInfo> what_signal = {
    {"SIGCHLD", SIGCHLD}, {"SIGTSTP", SIGTSTP}, {"SIGINT", SIGINT},
    {"SIGQUIT", SIGQUIT}, {"SIGTERM", SIGTERM}, {"SIGHUP", SIGHUP},
    {"SIGCONT", SIGCONT}, {"SIGPIPE", SIGPIPE}, {"SIGTTIN", SIGTTIN},
    {"SIGTTOU", SIGTTOU}, {"SIGUSR1", SIGUSR1}, {"SIGUSR2", SIGUSR2},
    // Tambahkan sinyal lain jika diperlukan (misalnya SIGWINCH, SIGURG)
};

/**
 * @brief Menunggu sebuah job (process group) di foreground hingga statusnya
 * berubah.
 * * Fungsi ini akan memblokir eksekusi hingga sebuah proses dalam process group
 * `pgid` berhenti (stopped), selesai (terminated), atau diinterupsi oleh
 * sinyal. Fungsi ini juga memastikan kontrol terminal dikembalikan ke shell
 * setelahnya.
...
int wait_for_job(pid_t pgid) {
  ...
  do {
    ...
    // Cek jika proses berhenti (WIFSTOPPED) atau selesai
    ...
    for (auto it = jobs.begin(); it != jobs.end();) {
      if (it->second.pgid == pid || it->second.pgid == getpgid(pid)) {
        if (WIFEXITED(status) || WIFSIGNALED(status)) {
          // Job completed
          it->second.status =
              WIFEXITED(status) ? JobStatus::EXITED : JobStatus::SIGNALED;
          it->second.term_status =
 ...
 */
int get_current_signal() { return current_signal; }

/**
 * @brief Reset variabel current_signal ke 0
 */
void reset_current_signal() { current_signal = 0; }
...
void setup_signals() {
  ...
  sigemptyset(&sa.sa_mask);
  sa.sa_flags =
      SA_RESTART | SA_NODEFER; // Tambahkan SA_NODEFER untuk menghindari rekursi
